...
class SiameseNetwork(pl.LightningModule):
    ...
    def __build_model(self):
        ...
        #embedding = torch.nn.Linear(nfeatures, self.hparams.embedding_dim) self.hparams.embedding_dim was 256 

        embedding = torch.nn.Sequential(
             torch.nn.Linear(nfeatures* 2, self.hparams.embedding_dim),
...
def parse_args():
    args = ArgumentParser()
    args.add_argument("-c", "--config", type=Path,default='config/siamese_resnet101.yml')
